 ...
 * 
//T1:大表
//TODO: 蓝牙FW/APP使用 --蓝牙Flash中维护的大表
__packed typedef struct _tag_HWTX_GroupInfo_BtAPP_
{
	unsigned short wCrcGrpInfoAllBtApp;	//本数据结构，除了wCrcGrpInfoAllBtApp之外所有数据的CRC值。(CRC计算：包括从dwSizeInByte开始，到最后一个成员的HWTX_MemberInfo_BtAPP内容）
	unsigned short wSizeInByte;	// 整个有效表的实际总大小：包括wCrcGrpInfoAllBtApp, dwSizeInByte~到最后一个成员的HWTX_MemberInfo_BtAPP内容。 
	unsigned char bDataType;		//本结构的数据类型. 应该是: HT_TYPE_BTAPP

	unsigned long dwRtcTime;		//时间戳。由领队APP每次修改本表时，写入当前系统时间。每次修改表元素，都需要更新为不同的时间戳。----重要，底层FW根据它来判断是否更新了新元素。
	//TODO: 应该bGrpCountDevice <= bGrpCountAttend <= bGrpCountTotal
	unsigned char bGrpCountDevice;	//小组领队宝同行宝总数。（设备总数：包含有领队宝设备、同行宝设备的所有成员。不含无设备的成员或未参加的成员） 
	unsigned char bGrpCountAttend;	//实际参加的总人数:含有设备的，无设备；
	unsigned char bGrpCountTotal;		//报名总人数。小组总人数：包括有领队宝设备的、有同行宝设备的、无同行宝但参加了本次活动的队员、报名但未参加的队员。
	HWTX_MemberInfo_BtAPP	tIndexMemberInfoArray[MAX_GRPCOUNT_SIZE];		//暂定这个最大值：以实际配置的为准
}HWTX_GroupInfoAll_BtAPP;
...
public class HwtxDataGroupInfoAllBtApp extends HwtxBluetoothSerializable {
	...
	public byte[] toBytes() {
		...
		bytes = HwtxCommandUtility.appendBytesToBytes(bytes, crcGrpInfoAllBtApp);
		...
		for (int i=0; ((i<indexMemberInfoArray.size()) && (i < HwtxCommandConstant.MAX_GRPCOUNT_SIZE)); ++i) {
			bytes = HwtxCommandUtility.appendBytesToBytes(bytes, indexMemberInfoArray.get(i).toBytes());
		}
	...
	public void fromBytes(byte[] bytes) {
		...
		crcGrpInfoAllBtApp  = HwtxCommandUtility.extractBytesFromBytes(bytes, start, crcGrpInfoAllBtApp.length);
		start += crcGrpInfoAllBtApp.length;
		
		sizeInByte  = HwtxCommandUtility.extractBytesFromBytes(bytes, start, sizeInByte.length);
		...
		rtcTime  = HwtxCommandUtility.extractBytesFromBytes(bytes, start, rtcTime.length);
		start += rtcTime.length;
		
		grpCountDevice  = HwtxCommandUtility.extractBytesFromBytes(bytes, start, grpCountDevice.length);
		start += grpCountDevice.length;
		

		grpCountAttend  = HwtxCommandUtility.extractBytesFromBytes(bytes, start, grpCountAttend.length);
		start += grpCountAttend.length;

		grpCountTotal  = HwtxCommandUtility.extractBytesFromBytes(bytes, start, grpCountTotal.length);
		...
		for (int i=0; i<arrayLength; ++i) {
			...
			byte memberInfoBtAppBytes[] = HwtxCommandUtility.extractBytesFromBytes(bytes, start, arrayItemLength);
	...
	public void setbDataType(Integer value) {
		this.bDataType = HwtxCommandUtility.int32ToBytes(value);
	}
	...
	public ArrayList<HwtxDataMemberInfoBtApp> getIndexMemberInfoArray() {
		if (indexMemberInfoArray.size() > HwtxCommandConstant.MAX_GRPCOUNT_SIZE) {
			return (ArrayList<HwtxDataMemberInfoBtApp>) indexMemberInfoArray.subList(0, HwtxCommandConstant.MAX_GRPCOUNT_SIZE);
		}
		else {
	...
	public void setIndexMemberInfoArray(ArrayList<HwtxDataMemberInfoBtApp> indexMemberInfoArray) {
		if (this.indexMemberInfoArray.size() < HwtxCommandConstant.MAX_GRPCOUNT_SIZE) {
			this.indexMemberInfoArray = indexMemberInfoArray;
		}
		else {
			this.indexMemberInfoArray = (ArrayList<HwtxDataMemberInfoBtApp>) indexMemberInfoArray.subList(0, HwtxCommandConstant.MAX_GRPCOUNT_SIZE);
		}
	}
