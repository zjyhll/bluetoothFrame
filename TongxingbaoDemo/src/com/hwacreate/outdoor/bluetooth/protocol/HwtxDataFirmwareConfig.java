...
//  设备flash中有默认出厂值；也可从手机领队APP中配置下来，保存在flash另一处，优先使用APP配置下来的有效配置
__packed typedef struct _tag_HWTX_FW_CONFIG_
{
	unsigned char bGpsInterval;	//GPS工作间隔，单位分钟，范围[1,60],default:5
	unsigned char bHktAtInterval;	//HktAt数传每轮间隔，单位分钟,范围[5,60]，default:5
	unsigned char bLostContactNum;	//失联次数,范围[1-10],default:3
	//TODO: wWarningDistanceX,告警距离，单位：米。范围[10, 5000] 
	//      上层APP需确保三个值依次增大: wWarningDistance1 <= wWarningDistance2 <= wWarningDistance3 
	unsigned short wWarningDistance1;	//告警距离1,最大告警距离--大于此距离黄色警告
	unsigned short wWarningDistance2;	//告警距离2,最大告警距离--大于此距离红色警告	
	unsigned short wWarningDistance3;	//告警距离3,最大告警距离--大于此距离失联警告	
	unsigned char	bWarningBatteryPercent; //电量百分比告警：电量少于该百分比，设备报警。 范围[0,100],default:15
	unsigned long myConfigCRC;	//本数据结构，除了myConfigCRC之外所有数据的CRC值。
}HWTX_FW_Config;
...
public class HwtxDataFirmwareConfig extends HwtxBluetoothSerializable {
	...
	public byte[] toBytes() {
		...
		bytes = HwtxCommandUtility.appendBytesToBytes(bytes, warningBatteryPercent);
	...
	public void fromBytes(byte[] bytes) {
		...
		bDataType  = HwtxCommandUtility.extractBytesFromBytes(bytes, start, bDataType.length);
		start += bDataType.length;
		
		gpsInterval  = HwtxCommandUtility.extractBytesFromBytes(bytes, start, gpsInterval.length);
		start += gpsInterval.length;
		
		hktAtInterval  = HwtxCommandUtility.extractBytesFromBytes(bytes, start, hktAtInterval.length);
		start += hktAtInterval.length;

		lostContactNum  = HwtxCommandUtility.extractBytesFromBytes(bytes, start, lostContactNum.length);
		start += lostContactNum.length;

		warningDistance1  = HwtxCommandUtility.extractBytesFromBytes(bytes, start, warningDistance1.length);
		start += warningDistance1.length;

		warningDistance2  = HwtxCommandUtility.extractBytesFromBytes(bytes, start, warningDistance2.length);
		start += warningDistance2.length;

		warningDistance3  = HwtxCommandUtility.extractBytesFromBytes(bytes, start, warningDistance3.length);
		start += warningDistance3.length;

		warningBatteryPercent  = HwtxCommandUtility.extractBytesFromBytes(bytes, start, warningBatteryPercent.length);
		start += warningBatteryPercent.length;

		configCRC  = HwtxCommandUtility.extractBytesFromBytes(bytes, start, configCRC.length);
	...
	public Integer getWarningDistance1Integer() {
		return Integer.valueOf(HwtxCommandUtility.bytesToInt16(warningDistance1));
	}

	public void setWarningDistance1Integer(Integer value) {
		this.warningDistance1 = HwtxCommandUtility.int16ToBytes(value.shortValue());
	}
	...
	public Integer getWarningDistance2Integer() {
		return Integer.valueOf(HwtxCommandUtility.bytesToInt16(warningDistance2));
	}

	public void setWarningDistance2Integer(Integer value) {
		this.warningDistance2 = HwtxCommandUtility.int16ToBytes(value.shortValue());
	}
	...
	public Integer getWarningDistance3Integer() {
		return Integer.valueOf(HwtxCommandUtility.bytesToInt16(warningDistance3));
	}

	public void setWarningDistance3Integer(Integer value) {
		this.warningDistance3 = HwtxCommandUtility.int16ToBytes(value.shortValue());
	}
	...
	public Integer getWarningBatteryPercentInteger() {
		return Integer.valueOf(HwtxCommandUtility.bytesToInt8(warningBatteryPercent));
	}

	public void setWarningBatteryPercentInteger(Integer value) {
		this.warningBatteryPercent = HwtxCommandUtility.int8ToBytes(value.byteValue());
	}
	...
	public void setConfigCRCInteger(Integer value) {
