package com.hwacreate.outdoor.push;
...
public class MyPushMessageReceiver extends PushMessageReceiver {
	...
	public void onBind(Context context, int errorCode, String appid, String userId, String channelId,
		...
		if (errorCode == 0) {
			// 绑定成功
			Log.d(TAG, "绑定成功");
			App.getInstance().setChannelId(channelId);
		}
	...
	public void onNotificationClicked(Context context, String title, String description, String customContentString) {
		...
		if (!TextUtils.isEmpty(customContentString)) {
			try {
				customJson = new JSONObject(customContentString);
				if (!customJson.isNull("act_id")) {
					mykey = "act_id";
					myvalue = customJson.getString("act_id");
				} else if (!customJson.isNull("club_id")) {
					mykey = "club_id";
					myvalue = customJson.getString("club_id");
				}
			} catch (JSONException e) {
				e.printStackTrace();
			}
		}
		App.getInstance().setPush(true);
		// Demo更新界面展示代码，应用请在这里加入自己的处理逻辑
		if (customContentString != null) {
			...
			if (TextUtils.equals(mykey, "act_id")) {
				if (TextUtils.equals(myvalue, App.getInstance().getHuoDongId())) {
					bundle.putInt("fromrenwu", CommonUtility.XianShiTab_RenWu);
				} else if (TextUtils.equals(myvalue, App.getInstance().getLeaderHuoDongId())) {
					bundle.putInt("fromrenwu", CommonUtility.XianShiTab_Leader_NOW);
				} else {
					bundle.putInt("fromrenwu", CommonUtility.XianShiTab_False);
				}
				bundle.putString("act_id", myvalue);
				intent.putExtras(bundle);
				intent.setClass(context.getApplicationContext(), HuoDongXiangQingActivity.class);
				context.getApplicationContext().startActivity(intent);
			} else if (TextUtils.equals(mykey, "club_id")) {
				bundle.putString("clubid", myvalue);
				intent.putExtras(bundle);
				intent.setClass(context.getApplicationContext(), JuLeBuXiangQingActivity.class);
				context.getApplicationContext().startActivity(intent);
			}
		} else {
			Intent intent = new Intent();
