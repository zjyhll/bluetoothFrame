...
public class ContactMyGuardianAcitivty extends BaseActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		parentView = getLayoutInflater().inflate(R.layout.leftfrag_myguardianfrg_tocontact, null);
	...
	public void onClick(View v) {
		...
		case R.id.btn_pop_ok:// TODO
			popContact.dismiss();
			final BluetoothDevice device = mLeDeviceListAdapter.getDevice(bluetoothPosition);
			if (device == null) {
				return;
			}
			if (guanlianshebei) {
				App.getInstance().setContactBleDuiYuanAddress(bluetoothdeviceName.substring(3));
			} else {
				if (fromTuZhong) {// 由途中跳入,关闭当前页面即可
					App.getInstance().setBleDuiYuanAddress(mAddress);
					App.getInstance().setBleDuiYuanName(bluetoothdeviceName);
				} else {// 由领队工具跳入
					App.getInstance().setBleLingDuiAddress(mAddress);
					App.getInstance().setBleLingDuiName(bluetoothdeviceName);
	...
	public class LeDeviceListAdapter extends BaseAdapter {
		...
		public View getView(final int i, View view, ViewGroup viewGroup) {
			...
			if (view == null) {
				...
				viewHolder.tv_tocontact = (TextView) view.findViewById(R.id.tv_tocontact);
			...
			view.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					ll_popup.startAnimation(
							AnimationUtils.loadAnimation(ContactMyGuardianAcitivty.this, R.anim.activity_translate_in));
