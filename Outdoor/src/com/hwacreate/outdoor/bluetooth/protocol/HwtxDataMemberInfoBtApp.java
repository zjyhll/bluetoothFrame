...
//户外同行：
//  同行队伍每个信息 (蓝牙FW与手机APP之间互享）
//		是否有设备、LDB/TXB模式、设备编号、设备SN串号、昵称、是否参加这次活动、是否启动APP图标、
__packed typedef struct _tag_HWTX_MemberInfo_BtAPP_
{
	HWTX_MemberInfo	tMemberInfo;
	char strNickName[MAX_NICKNAME_SIZE];	//昵称，最大10字节字母或5个汉字
	Member_MemberInfoApp_U tMemberInfoAPP;	//成员的APP额外信息，1Byte
}HWTX_MemberInfo_BtAPP;
...
public class HwtxDataMemberInfoBtApp extends HwtxBluetoothSerializable {
	...
	public byte[] toBytes() {
		...
		// 设置 tMemberInfo 
		bytes = HwtxCommandUtility.appendBytesToBytes(bytes, memberInfo.toBytes());
		
		// 设置 strNickName 
		bytes = HwtxCommandUtility.appendBytesToBytes(bytes, nickName);
		
		// 设置tMemberInfoAPP
		bytes = HwtxCommandUtility.appendBytesToBytes(bytes, memberInfoApp.toBytes());
	...
	public void fromBytes(byte[] bytes) {
		...
		byte[] memberInfoBytes = HwtxCommandUtility.extractBytesFromBytes(bytes, start, memberInfo.toBytes().length);
		memberInfo.fromBytes(memberInfoBytes);
		start += memberInfo.toBytes().length;

		nickName = HwtxCommandUtility.extractBytesFromBytes(bytes, start, nickName.length);
		start += nickName.length;
		
		byte[] memberInfoAppBytes = HwtxCommandUtility.extractBytesFromBytes(bytes, start, memberInfoApp.toBytes().length);
	...
	public String getNickNameString() {
		String utf8String = "  ";
		try {
			utf8String = new String(new String(nickName, "GB2312").getBytes("UTF-8"));
	...
	public void setNickNameString(String value) {
		...
		try {
			...
			this.nickName = HwtxCommandUtility.copyBytes(gb2312String.getBytes("GB2312"), this.nickName.length);
		} catch (UnsupportedEncodingException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
