...
public class HwtxCommandUtility {
	...
	public static byte[] setBitsOfBytes(byte[] original, Integer start, Integer length, byte[] value) {
	...
	// 将字节数组 original 的 index 指定的 bit 位设置为 bitValue，bitValue 只能为 0 和 1，任何不为 0 的值都当作 1 处理 
	public static byte[] setBitOfBytes(byte[] original, Integer index, Integer bitValue) {
	...
	public static byte[] extractBytesFromBytes(byte[] original, Integer start, Integer length){
	...
	public static byte[] extractBitsFromBytes(byte[] original, Integer start, Integer length) {
	...
	public static byte[] copyBytes(byte[] source, Integer targetLength) {
		...
		if (sourceLength > targetLength) {
			target = HwtxCommandUtility.extractBytesFromBytes(source, 0, targetLength);
		}
		else {
			target= HwtxCommandUtility.setBitsOfBytes(target, 0, source.length*8, source);
		}
	...
	public static byte[] leftShiftBytes(byte[] original, Integer length) {
		...
		for (int i=0; i<original.length; ++i) {
			...
			byte byteAfterShift = Integer.valueOf(Integer.rotateLeft(byteToShift, shiftBitsLength)).byteValue();
			tempAfterShift[i] = byteAfterShift;
		}
		
		System.out.println(Integer.toBinaryString(0xFFFF & HwtxCommandUtility.bytesToInt16(tempAfterShift)));
