...
/**
__packed typedef union _tag_MEMBER_DEVINFO_U_
{
	unsigned char v;
	__packed struct __tag_DEVINFO_T_
	{
		unsigned uDevNumInGroup : 5;   //该设备队员的组内编号，范围[1,30]	;
		unsigned uRev:1;							//Reserved bit，暂未定义
		unsigned uDeviceIsLDB : 1;    //本设备是否领队宝（APP确保一个队伍只有一个领队）,0-TXB同行宝（队员）,1-LDB领队宝(领队)；
		unsigned uDeviceReady : 1;    //是否有同行宝设备。 由领队APP设定：根据strDeviceSN是否为非零来判断，0-无同行宝；1-有同行宝
	}bit_info;
}Member_DevInfo_U;
