...
/**
//T2:表2
//TODO: 领队宝/同行宝MCU使用 --MCU维护的核心小表
__packed typedef struct _tag_HWTX_GroupInfoAll_
{
	unsigned short wCrcGrpInfoAll;	//本数据结构，除了wCrcGrpInfoAll之外所有数据的CRC值。(CRC计算：包括从dwSizeInByte开始，到最后一个成员的HWTX_MemberInfo内容）
	unsigned short wSizeInByte; // 整个有效表的实际总大小：包括wCrcGrpInfoAll, dwSizeInByte~到最后一个成员的HWTX_MemberInfo内容。 
	unsigned long dwRtcTime;		//时间戳，来自BtApp大表解析
	unsigned short wGroupID;	//组ID，由领队宝MCU生成，以领队宝的SN号作为随机种子。
	//TODO: bGrpTxbCount <=bGrpTotalMember
	unsigned char bGrpCountDevice;	//小组领队宝同行宝总数。（设备总数：包含有领队宝设备、同行宝设备的所有成员。不含无设备的成员或未参加的成员） 
	unsigned char bGrpCountAttend;	//(unused)：实际参加的总人数:含有设备的，无设备；
	unsigned char bGrpCountTotal;		//(unused)：报名总人数。小组总人数：包括有领队宝设备的、有同行宝设备的、无同行宝但参加了本次活动的队员、报名但未参加的队员。
	unsigned long dwGroupSeed;	//生成组ID的种子，由领队宝SN相关	
	HWTX_MemberInfo_Map tIndexMemberInfoMapArray[MAX_GRPCOUNT_DEV_SIZE];
}HWTX_GroupInfoAll;
 */
public class HwtxDataGroupInfoAll extends HwtxBluetoothSerializable {
	...
	public byte[] toBytes() {
		...
		for (int i=0; ((i<indexMemberInfoMapArray.size()) && (i < HwtxCommandConstant.MAX_GRPCOUNT_DEV_SIZE)); ++i) {
			bytes = HwtxCommandUtility.appendBytesToBytes(bytes, indexMemberInfoMapArray.get(i).toBytes());
		}
	...
	public void fromBytes(byte[] bytes) {
		...
		groupID  = HwtxCommandUtility.extractBytesFromBytes(bytes, start, groupID.length);
		...
		groupSeed  = HwtxCommandUtility.extractBytesFromBytes(bytes, start, groupSeed.length);
		...
		for (int i=0; i<arrayLength; ++i) {
			...
			byte memberInfoMapBytes[] = HwtxCommandUtility.extractBytesFromBytes(bytes, start, arrayItemLength);
	...
	public ArrayList<HwtxDataMemberInfoMap> getIndexMemberInfoMapArray() {
		if (indexMemberInfoMapArray.size() > HwtxCommandConstant.MAX_GRPCOUNT_DEV_SIZE) {
			return (ArrayList<HwtxDataMemberInfoMap>) indexMemberInfoMapArray.subList(0, HwtxCommandConstant.MAX_GRPCOUNT_DEV_SIZE);
		}
		else {
	...
	public void setIndexMemberInfoMapArray(ArrayList<HwtxDataMemberInfoMap> indexMemberInfoMapArray) {
		if (this.indexMemberInfoMapArray.size() < HwtxCommandConstant.MAX_GRPCOUNT_DEV_SIZE) {
			...
			this.indexMemberInfoMapArray = (ArrayList<HwtxDataMemberInfoMap>) indexMemberInfoMapArray.subList(0, HwtxCommandConstant.MAX_GRPCOUNT_DEV_SIZE);
		}
	}
