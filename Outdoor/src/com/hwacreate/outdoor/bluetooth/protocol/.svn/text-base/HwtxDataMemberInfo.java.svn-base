...
//TODO：一台同行宝的最小信息
__packed typedef struct _tag_HWTX_MEMBERINFO_
{
	Member_DevInfo_U tDevInfo;		//成员的设备信息，1Byte
	unsigned char strDeviceSN[MAX_DEVSN_SIZE]; //唯一的设备SN串号，固定6字节的数字，由设备量产时写入。有非0值，则表示该队员有同行宝；如果为0值，则表示该队员无同行宝硬件。
}HWTX_MemberInfo;
...
public class HwtxDataMemberInfo extends HwtxBluetoothSerializable {
	...
	public void fromBytes(byte[] bytes) {
		...
		byte[] devInfoBytes = HwtxCommandUtility.extractBytesFromBytes(bytes, start, devInfo.toBytes().length);
		devInfo.fromBytes(devInfoBytes);
		start += devInfo.toBytes().length;

		deviceSn = HwtxCommandUtility.extractBytesFromBytes(bytes, start, deviceSn.length);
	}
	...
	public void setDeviceSnString(String value) {
		this.deviceSn = HwtxCommandUtility.copyBytes(value.getBytes(), this.deviceSn.length);
	}
