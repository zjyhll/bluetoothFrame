...
public class HwtxDataGpsInfoDataComp extends HwtxBluetoothSerializable {
	...
	public void fromBytes(byte[] bytes) {
		...
		gpsMisc  = HwtxCommandUtility.extractBytesFromBytes(bytes, start, gpsMisc.length);
		start += gpsMisc.length;
		
		gpsTime  = HwtxCommandUtility.extractBytesFromBytes(bytes, start, gpsTime.length);
		start += gpsTime.length;

		gpsLatitude  = HwtxCommandUtility.extractBytesFromBytes(bytes, start, gpsLatitude.length);
		start += gpsLatitude.length;
		
		gpsLongitude  = HwtxCommandUtility.extractBytesFromBytes(bytes, start, gpsLongitude.length);
	}
