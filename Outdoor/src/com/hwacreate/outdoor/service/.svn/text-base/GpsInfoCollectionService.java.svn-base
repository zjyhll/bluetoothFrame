...
public class GpsInfoCollectionService extends Service {
	...
	public void onCreate() {
		...
		info = new GpsInfo();
		mGpSinfoDao = new BaseDao(info, getApplicationContext());
		// 在接收gps数据之前接收之前的数据
		try {
			if (mGpSinfoDao.searchBylatItemId(gps_start_time) != null) {
				if (mGpSinfoDao.searchBylatItemId(gps_start_time)
						.getLocationInfo() != null) {
					arrayLocationInfo = new JSONArray(mGpSinfoDao
							.searchBylatItemId(gps_start_time)
							.getLocationInfo());
				} else {
		...
		public void onReceiveLocation(BDLocation location) {
			...
			if (longitude == location.getLongitude()
				...
				try {
					...
					if (mGpSinfoDao != null) {
						int temp = mGpSinfoDao.updateLatItem(
								arrayLocationInfo.toString(), gps_start_time);
						if (temp == 0) {// 当返回为0时说明数据库中没有该条，即创建新的
